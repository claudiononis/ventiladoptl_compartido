sap.ui.define([
  ...
  return Controller.extend("ventilado.ventiladoptl.controller.Desconsolidado", {
    onInit: function () {
      this._dbConnections = [];
    ...
    ejecutarAcciones: function () {
      ...
      return this.obtenerYProcesarDatos()
        .then(() => {
          ...
          const yaAsignadoAlguno = ctx.datosD.some(item => item.contenedor && item.display);

          if (yaAsignadoAlguno) {
            ctx.datosD = this.asignarContenedoresYDisplays(ctx.datosD, limite1, limite2, usarRoll, true, false);
          }
          else {
              ...
              beginButton: new sap.m.Button({
                ...
                press: () => {
                  ...
                  if (excedeAhora) {
                    ...
                    sap.m.MessageToast.show("Asignacion recalculada correctamente.");
                  }
    ...
    determinarContenedor: function (cubTeo) {
      ...
      if (cubTeo <= limite1) {
        return "CUB";
      } else if (localStorage.getItem("usarRoll") === "true" && cubTeo > limite1 && cubTeo <= limite2) {
        return "ROLL";
      } else {
        return "PALLET";
      }
    ...
    onChangeContenedor: function (oEvent) {
      if (localStorage.getItem("asignacionConfirmada") === "true") {
        sap.m.MessageToast.show("Asignacion bloqueada. Activar modo administrador.");
        return;
      }
    ...
    onConfirmarContenedores: function () {
      this.getView().getModel().setProperty("/contenedoresHabilitados", false);
    ...
    procesarDatos: function (datos) {
      ...
      datos.forEach((registro) => {
        ...
        const displayFormatted = registro.Prodr ? "dsp-" + ("000" + registro.Prodr).slice(-3) : "";
        ...
        if (!resultado[ruta]) {
          ...
          if (registro.Prodv && resultado[ruta].contenedor === "") {
            resultado[ruta].contenedor = contenedorMap[registro.Prodv] || "";
          }
      ...
      const dialog = new sap.m.Dialog({
        ...
        beginButton: new sap.m.Button({
          ...
          press: () => {
            ...
            if (clave === "12345") { // 🔐 Cambiar por tu clave real si querés
              ...
              oView.getModel().setProperty("/contenedoresHabilitados", true);
