    ...
    return Controller.extend("ventilado.ventiladoptl.controller.Scan2", {
        ...
        onPasswordSubmit: function () {
            ...
            if (sPassword === "12345") {  // Reemplazar con la lógica real de validación
                ...
                MessageBox.error("Error :Contrasena incorrecta. Por favor, intentelo de nuevo.", {
                    title: "Error ",
                    styleClass: "customMessageBox", // Aplica la clase CSS personalizada
                    onClose: function () {
                        console.log("Mensaje de error personalizado cerrado.");
        ...
        */
                    ...
                    .then(function (data) {
                        if (data.estado && Array.isArray(data.estado)) {
                            data.estado.forEach(function (ruta) {
                                ...
                                if (ruta.completada) {//&& !ctx._rutasConfirmadas.has(String(parseInt(ruta.rutaId, 10)))) {
                                    ctx.procesarRutaConfirmadaDesdePTL(ruta);
                                    ctx._rutasConfirmadas.add(String(parseInt(ruta.rutaId, 10)));
                                }
                            });
                        }
                    })
        ...
        procesarRutaConfirmadaDesdePTL: async function (ruta) {
            ...
            const registro = tableData.find((item) => item.Ruta === String(parseInt(ruta.rutaId, 10)));
            if (!registro) {
            ...
            // ------------------------------------------------------------------
            const idx = tableData.findIndex(item => item.Ruta === String(parseInt(ruta.rutaId, 10)));//ruta.rutaId);   // posición en el array
            ...
            const id = await this.buscarIdEnIndexedDBPorRuta(String(parseInt(ruta.rutaId, 10)));  // Implementado más abajo
            if (!id) {
        ...
        verificarCicloCompletado: function () {
            ...
            const rutasPendientes = tableData.filter(item => item.FALTA != "0" || item.FALTA != 0);
